export const habilidadesDuras = [
  {
    habilidad: "HTML y CSS",
    porcentaje: 90,
    tooltip: [
      '"Las raices del hermoso árbol que veo crecer."',
      'Aprendí en "freeCodeCamp" y llevando la teoria a la práctica en cada desafío.',
    ],
  },
  {
    habilidad: "JavaScript",
    porcentaje: 75,
    tooltip: [
      '"El tallo del árbol rompiendo para crecer."',
      '"Crecer duele" dice el dicho y JavaScrip me lo demostró, pero siempre conciente de que era fundamental en el proceso.',
    ],
  },
  {
    habilidad: "React",
    porcentaje: 85,
    tooltip: [
      '"Lluvia refrescante."',
      'React trajo calma, orden, permanencia y "las primeras flores" que se convertirian en frutos mañana.',
    ],
  },
  {
    habilidad: "SASS",
    porcentaje: 90,
    tooltip: [
      '"Maduración"',
      "Si el sol es fundamental en el proceso de florecer y madurar los frutos, SCSS es un gran sol en mis proyectos. A diario salgo a su encuentro para ver que más puedo aprender. :)",
    ],
  },
];
export const habilidadesBlandas = [
  {
    habilidad: "Autogestión",
    porcentaje: 90,
    tooltip: [
      '"¡Tormentas!"',
      "Aunque las hojas y los frutos puedan caer, la nutrición diaria permite que el árbol se mantenga fuerte, paciente y perseverante.",
    ],
  },
  {
    habilidad: "Curiosidad",
    porcentaje: 95,
    tooltip: [
      '"¡El cielo es el límite!"',
      "La pasión por aprender, el poder de la investigación y la exploración de nuevas formas de hacer las cosas son el camino hacia las metas.",
    ],
  },
  {
    habilidad: "Creatividad",
    porcentaje: 90,
    tooltip: [
      '"¡Reinvéntate!"',
      "La creatividad es siempre una forma de reinvención: implica pensar más allá de los límites establecidos, aplicar la intuición y avanzar desde lo simple hacia lo complejo.",
    ],
  },
  {
    habilidad: "Trabajo en equipo",
    porcentaje: 35,
    tooltip: [
      '"Un árbol que creció en soledad"',
      "No conoce la experiencia de formar parte de la frondosa arboleda, pero está dispuesto a aprender a ser parte de ella. :)",
    ],
  },
];
